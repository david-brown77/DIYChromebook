...
function fClearAgedDirs () {
  ...
  if [ `echo "$vDesiredFreeSpace $vCurrentFreeSpace" | awk '{print ($1 > $2)}'` -eq 1 ];
    ...
    then
      vNowMessage=`date +%Y``date +%m``date +%d``date +%H``date +%M``date +%S`" - ""$vCurrentFreeSpace"" is less then ""$vDesiredFreeSpace"" so removing old GoogleUser direc$
      echo "$vNowMessage"
      echo "$vNowMessage" 1>> "$vLogFileOutputFullPath" 2>> "$vLogFileErrorFullPath"
      ...
      if [ `echo "$vDesiredFreeSpace $vCurrentFreeSpace" | awk '{print ($1 < $2)}'` ] && ! (ls /home/googleuser-aging-* &> /dev/null) ;
        then
          vNowMessage=`date +%Y``date +%m``date +%d``date +%H``date +%M``date +%S`" - WARNING!!! ""$vCurrentFreeSpace"" is less then ""$vDesiredFreeSpace"" of desired free s$
          echo "$vNowMessage"
          echo "$vNowMessage" 1>> "$vLogFileOutputFullPath" 2>> "$vLogFileErrorFullPath"
      fi
    ...
    # enough free space, so just log that it was checked
      ...
      vNowMessage=`date +%Y``date +%m``date +%d``date +%H``date +%M``date +%S`" - Nothing to process, free space is ""$vCurrentFreeSpace"" and desired free space is ""$vDesi$
      echo "$vNowMessage"
      echo "$vNowMessage" 1>> "$vLogFileOutputFullPath" 2>> "$vLogFileErrorFullPath"
  fi
}
